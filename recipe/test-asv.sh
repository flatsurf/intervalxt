...
if test "x$have_asv" = "xyes"; then
  git clone git@github.com:flatsurf/intervalxt-asv.git .asv/results
else
  git clone https://github.com/flatsurf/intervalxt-asv.git .asv/results
fi
...
if test "x$have_asv" = "xyes"; then
  ...
  git push git@github.com:flatsurf/intervalxt-asv.git gh-pages:gh-pages -f
fi
