...
std::vector<mpq_class> LengthArithmetic<T>::coefficients(const T& value) {
  if constexpr (std::is_same_v<T, long long> || std::is_same_v<T, unsigned long long>) {
    return {mpq_class(std::to_string(value))};
  } else {
    return {value};
  }
}
...
template struct LengthArithmetic<int>;
template struct LengthArithmetic<unsigned int>;
template struct LengthArithmetic<long>;
template struct LengthArithmetic<unsigned long>;
template struct LengthArithmetic<long long>;
template struct LengthArithmetic<unsigned long long>;
}
