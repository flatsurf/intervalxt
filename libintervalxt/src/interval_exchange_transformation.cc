...
bool IntervalExchangeTransformation::boshernitzanNoSaddleConnection(const Label& top, const Label& bottom) const {
  ...
  std::vector<mpq_class> values = self->translation(
      *++std::find(begin(self->top), end(self->top), top),
      *++std::find(begin(self->bottom), end(self->bottom), bottom),
      self->labelToCoefficients());
