...
class RationalLinearSubspace : boost::equality_comparable<RationalLinearSubspace> {
 public:
  ...
  RationalLinearSubspace(const std::vector<std::vector<mpq_class>>& generators);

  // The subspace which satisfies Σa_i x_i = y for each vector (a_0, …, a_n) in
  // constraints.
  RationalLinearSubspace(const std::vector<std::vector<mpq_class>>& equations, const std::vector<mpq_class>& y);
